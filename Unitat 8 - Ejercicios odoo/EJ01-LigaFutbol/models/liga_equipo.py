...
class LigaEquipo(models.Model):
    ...
    #Variable para contar cuantos partidos han ganado con diferencia
    ganarDiferencia=fields.Integer(default=0)
        #Variable para contar cuantos partidos han perdido con diferencia
    perderDiferencia=fields.Integer(default=0)
    ...
    def _compute_puntos(self):
        for record in self:
            # Resto los partidos ganados con diferencia de las victorias porque las victorias solo deben multiplicarse por 3
            # las victorias ganadas con diferencia deben multiplicarse por 4
            # Sumamos el empate y restamos 1 si perdemos por diferencia
            record.puntos = ((record.victorias - record.ganarDiferencia) * 3)+(record.ganarDiferencia*4) + record.empates - record.perderDiferencia
