...
class LigaPartido(models.Model):
    ...
    def actualizoRegistrosEquipo(self):
        ...
        for recordEquipo in self.env['liga.equipo'].search([]):
            ...
            recordEquipo.ganarDiferencia=0
            recordEquipo.perderDiferencia=0
            ...
            for recordPartido in self.env['liga.partido'].search([]):  
                ...
                if recordPartido.equipo_casa.nombre==recordEquipo.nombre:
                    ...
                    # añadimos la cantidad de veces que el equipo de casa pierde o gana por diferencia  
                    if (recordPartido.goles_casa-recordPartido.goles_fuera)>=4:
                        recordEquipo.ganarDiferencia += 1
                    elif (recordPartido.goles_fuera-recordPartido.goles_casa)>=4:
                        recordEquipo.perderDiferencia += 1
                ...
                if recordPartido.equipo_fuera.nombre==recordEquipo.nombre:
                    ...
                    # añadimos la cantidad de veces que el equipo de fuera pierde o gana por diferencia
                    if (recordPartido.goles_casa-recordPartido.goles_fuera)>=4:
                        recordEquipo.perderDiferencia += 1
                    elif (recordPartido.goles_fuera-recordPartido.goles_casa)>=4:
                        recordEquipo.ganarDiferencia += 1
    ...
    # Botón para añadir 2 goles al equipo de casa    
    def sumarDosCasa(self):
        for recordEquipo in self.env['liga.equipo'].search([]):
            for recordPartido in self.env['liga.partido'].search([]):
                if recordPartido.equipo_casa.nombre==recordEquipo.nombre:  
                    recordPartido.goles_casa+=2
        self.actualizoRegistrosEquipo() # Llamamos a la función para actualizar los registros
        
    # Botón para añadir 2 goles al equipo de fuera 
    def sumarDosVisitante(self):
        for recordEquipo in self.env['liga.equipo'].search([]):
            for recordPartido in self.env['liga.partido'].search([]):
                if recordPartido.equipo_fuera.nombre==recordEquipo.nombre:  
